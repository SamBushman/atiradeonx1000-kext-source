...
### ATIR500DVDContext (20 missing)
`build_scissor`, `check_stamps`, `clientMemoryForType`, `discard_command_buffer`, `dvd_enable_deint`, `dvd_enable_overlay`, `dvd_setup_overlay`, `dvd_setup_subpicture`, `finish`, `getTargetAndMethodForIndex`, `invalidate`, `read_regs`, `setup_buffers`, `show_buffer`, `start`, `stop`, `submit_context_buffer`, `update_surface`, `wait_for_stamps`, `write_regs`
...
### IOATIR5002DContext (23 missing)
`clientClose`, `clientMemoryForType`, `create_image`, `create_shared`, `create_transfer`, `declare_image`, `delete_image`, `finish`, `get_config`, `get_surface_info`, `init_command_buffer_header`, `lock_memory`, `remove_surface`, `scale_surface`, `set_macrovision`, `set_surface`, `set_surface_paging_options`, `set_surface_vsync_options`, `start`, `stop`, `swap_surface`, `unlock_memory`, `wait_image`
...
### IOATIR500DVDContext (17 missing)
`clientClose`, `clientMemoryForType`, `create_shared`, `declare_image`, `delete_image`, `finish`, `get_config`, `get_status`, `get_surface_size`, `init_command_buffer_header`, `lock_all_buffers`, `remove_surface`, `setCompatibleSurfaceMode`, `start`, `stop`, `unlock_memory`, `write_buffer`
