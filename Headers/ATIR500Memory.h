...
/*
 * CORRECTED (issue #58 follow-up): this class IS an OSObject subclass in
 * the real kext (real symbols `ATIR500Memory::MetaClass`,
 * `ATIR500Memory::gMetaClass`, `getMetaClass`, `MetaClass::alloc`), but was
 * previously modelled as a plain class. That left the OSObject constructor
 * hand-called on a non-OSObject and the metaclass global (`gMetaClass`)
 * undefined, and made every `vtable[0x48/4]` call hit the wrong table.
...
public:
     ...
     * (allocation-site cross-check, issue #23). Now generated by
     * `OSDefineMetaClassAndStructors` (see the class comment above).
     */
     ...
     *
     * CORRECTED (issue #58 follow-up): the "+0x48 call" is the qualified
     * base-class call `OSObject::init()` - the raw decompile's `(*pcRam00000048)()`
     * is an indirect call through the OSObject vtable symbol (the Mach-O
     * relocation at 0x18d54 targets `__ZTV8OSObject`; slot +0x48 of the real
     * kernel's `OSObject` vtable, read statically from /mach_kernel, is
     * `OSObject::init()`). The earlier issue #52 conclusion that this slot is
     * `OSObject::_RESERVEDOSObject0()` was wrong (that slot is +0x50), so the
     * "gate is a decompiler artifact / genuine no-op" account no longer
     * applies: `init()` really is `if (!OSObject::init()) return false;` followed
     * by tearing down any existing chunk list (a re-init reset). No real call site for this
     ...
     * Both real bodies: call this object's own virtual `init()` first (a
     * genuine vtable call through `this`, slot +0x48 of ATIR500Memory's own
     * vtable - CORRECTED, issue #58 follow-up; issue #52 had misread the
     * target, see `init`'s comment above); on success, allocate the pool's
     ...
     * target `IOFreeAligned`), then calls `OSObject::free()` as its final
     * real action - CORRECTED, issue #58 follow-up: the relocation at 0x1916c
     * targets `__ZTV8OSObject` and slot +0x4c of the real kernel's OSObject
     * vtable is `OSObject::free()` (issue #52's `_RESERVEDOSObject1` reading was
     * wrong; that slot is +0x54). This is the standard "free my resources, then
     * super::free()" shape. */
    virtual void free();
