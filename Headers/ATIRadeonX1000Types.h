...
struct ATIR500SurfaceBuffer {
    ...
    UInt32  hzBlockField;        /* +0x28, RENAMED from the incorrect `formatOrTilingBits` (real naming/scope error, CAUGHT AND FIXED via a targeted follow-up investigation): decompiling `page_off_texture` directly (real addr 0x9740, never actually transcribed before - see `Sources/IOATIR500GLContext_PageOffTexture.cpp`) showed its own "+0x28" read is on a COMPLETELY DIFFERENT struct - the per-face mip sub-record reached via `texture+0x14 + face*2` (the SAME real `faceMip` structure `ATIR500GLContext_TextureLoad.cpp` already names, where `+0x28` is the already-confirmed `loadedBits` field) - not this struct at all. This struct's OWN real `+0x28` is a packed HZMEM block-handle field: CONFIRMED by `ATIR500Surface_AllocSurfaceBuffer.cpp`'s own real read/write of it as an `HZMEM_Alloc` block-handle slot, independently cross-confirmed by `ATIR500Surface_ResolveFSAABuffer.cpp`'s own already-committed real usage (passed directly as `HZMEM_GetBlockOffset`/`HZMEM_GetBlockCount`'s own `surfaceHzField` parameter) - the `0x3ff00000` sentinel mask that code already checks against is EXACTLY blockKind 2's real 10-bit index field (bits 20-29), matching `HZMEM_Free`'s own confirmed bit-layout (issue #28) precisely. */
    UInt32  hzBlockExtra;        /* +0x2c, NEWLY IDENTIFIED (issue #47, decompiling `ATIR500Surface::load_3d_blit`) - previously left as unnamed padding between `hzBlockField` and `hzBlockCountField`, but `load_3d_blit` reads a real full dword here and writes it straight into a PM4 register slot (index 0x13ca). Real role beyond "a real per-surface dword `load_3d_blit` forwards verbatim" INFERRED, not confirmed by any other reader in this project yet. */
    UInt32  hzBlockCountField;   /* +0x30, CONFIRMED (issue #13, ATIR500Surface::resolve_fsaa_buffer): real value read directly as an operand to a `0x1385`-tagged burst slot right after an `HZMEM_GetBlockCount` call on this same struct's `hzBlockField` - almost certainly a real HyperZ block-count-adjacent field, exact semantics beyond that not independently confirmed. */
