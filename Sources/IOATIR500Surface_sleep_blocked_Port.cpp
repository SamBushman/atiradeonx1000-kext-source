/*
 * IOATIR500Surface_sleep_blocked_Port.cpp
 *
 * IOATIR500Surface::sleep_blocked (real addr 0x11840, 0 bytes) - mechanically ported from the Ghidra decompile of the shipped kext (Tools/replace_fn.py /
...
void IOATIR500Surface::sleep_blocked() {
  ...
  IVar1 = self[0xbf6];
  while (IVar1 != 0x0) {
    GH_IOLockSleep(M<UInt32>(self + 0xc04),self,0);
    IVar1 = self[0xbf6];
  }
