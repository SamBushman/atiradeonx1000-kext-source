...
void IOATIR500GLContext::stop(IOService *provider) {
    UInt8 *self = reinterpret_cast<UInt8 *>(this);

    W(self, 0x294) = 0;

    UInt8 *accel = reinterpret_cast<UInt8 *>(accelerator);
    if (reinterpret_cast<UInt8 *>(W(accel, 0x60)) == self) {
        W(accel, 0x60) = W(self, 0x80); /* liveGLContextListHead = nextLiveContext */
        if (W(accel, 0x60) == 0 && W(accel, 0x68) == 0) {
            typedef void (*Fn0x538)(void *);
            void **vtable = *reinterpret_cast<void ***>(accel);
            reinterpret_cast<Fn0x538>(vtable[0x538 / 4])(accel);
        }
    } else {
        /* real: walk from the list head looking for the node whose own
           `nextLiveContext` is `this`, then splice it out. */
        UInt8 *predecessor = reinterpret_cast<UInt8 *>(W(accel, 0x60));
        for (;;) {
            UInt8 *next = reinterpret_cast<UInt8 *>(W(predecessor, 0x80));
            if (next == nullptr || next == self) {
                break;
            }
            predecessor = next;
        }
        W(predecessor, 0x80) = W(self, 0x80);
    }

    W(accel, 0x738) = W(accel, 0x738) - 1; /* real: see honest note in file header re: start()'s different +0x738 object */

    W(accel, 0x5c8) = 0x20000; /* cachedBufferSizeThreshold */
    freeCommandBuffer();
    W(accel, 0x5d8) = 0x10000; /* cachedBufferSizeThresholdB */

    UInt8 *node = reinterpret_cast<UInt8 *>(W(accel, 0x5cc)); /* freeBufferListHead */
    while (node != nullptr) {
        UInt8 *mapping = reinterpret_cast<UInt8 *>(W(node, 0x10)); /* VendorTextureBuffer::gartMapping */
        if (mapping != nullptr) {
            typedef UInt32 (*IdentFn)(void *);
            void **vtable = *reinterpret_cast<void ***>(mapping);
            UInt32 ident = reinterpret_cast<IdentFn>(vtable[0xdc / 4])(mapping);
            if (ident == W(self, 0x78)) {
                ReleaseObj(mapping);
                W(node, 0x10) = 0;
            }
        }
        node = reinterpret_cast<UInt8 *>(W(node, 0x3c)); /* VendorTextureBuffer::streamListNext */
    }

    UInt8 *buf = reinterpret_cast<UInt8 *>(W(self, 0xe8));
    if (buf != nullptr) {
        for (;;) {
            UInt8 *next = reinterpret_cast<UInt8 *>(W(buf, 0x3c));
            accel = reinterpret_cast<UInt8 *>(accelerator);
            if (W(accel, 0x5d4) < 0x10) { /* freeBufferListCount */
                if (W(accel, 0x5d0) == 0) {
                    W(accel, 0x5cc) = reinterpret_cast<UInt32>(buf); /* freeBufferListHead = buf */
                } else {
                    W(reinterpret_cast<UInt8 *>(W(accel, 0x5d0)), 0x3c) = reinterpret_cast<UInt32>(buf);
                }
                W(accel, 0x5d0) = reinterpret_cast<UInt32>(buf); /* freeBufferListTail = buf */
                W(buf, 0x3c) = 0;
                W(accel, 0x5d4) = W(accel, 0x5d4) + 1;
                buf[0x54] = 0; /* real "resident/in-flight" byte flag, see file header comment - NOT the +0x54 linkedBuffer pointer */
                if (W(buf, 0x10) != 0) {
                    ReleaseObj(reinterpret_cast<void *>(W(buf, 0x10)));
                    W(buf, 0x10) = 0;
                }
            } else {
                accelerator->freeOneDataBuffer(reinterpret_cast<VendorTextureBuffer *>(buf));
            }
    ...
    accel = reinterpret_cast<UInt8 *>(accelerator);
    while (W(accel, 0x5d4) > 0xf) {
        UInt8 *evict = reinterpret_cast<UInt8 *>(W(accel, 0x5cc));
        W(accel, 0x5cc) = W(evict, 0x3c);
        W(accel, 0x5d4) = W(accel, 0x5d4) - 1;
        accelerator->freeOneDataBuffer(reinterpret_cast<VendorTextureBuffer *>(evict));
        accel = reinterpret_cast<UInt8 *>(accelerator);
    }
    ...
    if (W(self, 0xd4) != 0) {
        accelerator->freeCommandBuffer(reinterpret_cast<VendorCommandBuffer *>(self + 0xcc));
    }
    if (W(self, 0xb8) != 0) {
        ReleaseObj(reinterpret_cast<void *>(W(self, 0xb8)));
    }
    if (reinterpret_cast<UInt8 *>(W(accel, 0x78)) == self) {
        W(accel, 0x78) = 0;
    }
    if (W(self, 0x290) != 0) {
        IOATIR500Surface *surface = reinterpret_cast<IOATIR500Surface *>(W(self, 0x290));
        surface->remove_gl_context_from_list(this);
    ...
    if (W(self, 0x88) != 0) {
        for (UInt8 *slot = self; slot != self + 0xa8; slot += 4) {
            if (W(slot, 0x2a4) != 0) {
                UInt8 *tex = reinterpret_cast<UInt8 *>(W(slot, 0x2a4));
                void *rec = reinterpret_cast<void *>(W(tex, 0x14));
                UInt32 wasLast = FUN_00008340(reinterpret_cast<UInt8 *>(rec) + 0x10);
                if (wasLast == 1) {
                    clientHandle->delete_texture(reinterpret_cast<VendorTextureBuffer *>(tex));
                }
                W(slot, 0x2a4) = 0;
            }
        }
        ReleaseObj(clientHandle);
        W(self, 0x88) = 0;
    }
