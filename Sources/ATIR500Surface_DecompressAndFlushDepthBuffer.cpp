 ...
 * - Otherwise: calls the real, previously-undeclared method
 *   `load_3d_blit` (real mangled name confirmed via Ghidra's own
 *   decompile signature; own body NOT decompiled this pass - forward-
 *   declared below) to emit the real depth-decompress PM4 sequence
 *   directly into `record + param2`, returns the resulting dword index
 ...
 * pointers throughout this project. `load_3d_blit`'s own body remains a
 * genuine gap (real name/existence confirmed, real behavior not
 * independently decompiled). No C++ compiler was available in the
...
/* real name/existence/addr (0x3d4c0) CONFIRMED via `nm` this pass, real behavior not decompiled this pass */
extern "C" UInt32 *ATIR500Surface_load_3d_blit(ATIR500Surface *self, UInt32 *dest, ATIR500SurfaceBuffer *scratch) asm("__ZN14ATIR500Surface12load_3d_blitEPmP20ATIR500SurfaceBuffer");
...
UInt32 ATIR500Surface::decompress_and_flush_depth_buffer(ATIR500SurfaceBuffer *scratch, UInt32 param2, UInt32 *record) {
    ...
    UInt32 *end = ATIR500Surface_load_3d_blit(this, record + param2, scratch);
