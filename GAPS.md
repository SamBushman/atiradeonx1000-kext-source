...
| Userspace binaries: linking (issue #61) | All five link and load with the stock's exact export sets; behaviour verified for libGL, GA, VA and glprog's ARB parser (78 parses); GLDriver is load-only (needs kext+display). Every function of all five binaries is transcribed and linked (GLDriver `FUN_00115fa0` included; no stubs), the dropped-call-argument defect is fixed and counted (`Userspace/README.md`, "Call-argument accuracy"). Not covered: behaviour of the GLSL front end and the rest of glprog (#60), GLDriver behaviour (kext + display), C++ exceptions in recompiled images |
