...
| Userspace binaries: linking (issue #61) | libGL, GA, VA link and load (behavioural checks pass); libGLProgrammability links and loads with the stock's exact export set but its ARB program parser is not yet equivalent (`Tests/userspace/README.md`); GLDriver not linked end to end. See `Userspace/README.md`, "Linking" |
